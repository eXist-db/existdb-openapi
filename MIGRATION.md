# Migration: `lsp` → `langservice` + `cursor` (pre-1.0.0)

This document describes the breaking rename landed in PR for issue #15. It applies to consumers that integrated against pre-release versions of exist-api (formerly the `lsp:` namespace inherited from `exist-lsp`).
...
The previous `lsp:` namespace and `/api/lsp/*` URL prefix were inherited from the migrated `exist-lsp` standalone package. They implied that exist-api speaks the Language Server Protocol — it does not. exist-api is an HTTP/JSON backend whose data shapes are *inspired by* LSP types but consumed via REST, not JSON-RPC. See issue #7 for the full discussion.
...
- The `repoxml` package name / abbrev (still `exist-api` until the repo rename in #13 lands).
